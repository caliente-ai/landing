...
1. <https://vercel.com>, then **Add New... > Project > Import Git Repository**.
...
4. **Deploy.** The first build takes about a minute. You get a `*.vercel.app`
   URL; open it to confirm the page renders.

Every push to `main` now auto-deploys to production.

## 2. Add the domain in Vercel

1. Project, then **Settings > Domains**.
2. Add **`caliente-ai.com`**. When prompted, also add **`www.caliente-ai.com`**
   and let Vercel redirect `www` to the apex.
3. Vercel shows the exact DNS records to create. They should match step 3.

## 3. Add the web records in Namecheap

Namecheap, then **Domain List > Manage `caliente-ai.com` > Advanced DNS**.

**Remove the parking defaults** (they collide with the records below):

- `CNAME Record` for Host `www` pointing to `parkingpage.namecheap.com`
- `URL Redirect Record` (or `A Record`) for Host `@` pointing to Namecheap parking
...
Save (the green checkmark). If Vercel's dashboard shows a different IP or
target, use the value Vercel displays.
